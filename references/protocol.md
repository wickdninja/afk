...
- `channel_id` / `thread_ts` — transport-specific addressing, but almost every chat transport uses a channel + thread pair. For SMS/email: see their references.
...
- `last_seen_ts` — newest consumed message ts. `oldest` param on read-thread uses this.
